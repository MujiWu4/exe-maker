...
def match_stock(purchase_row, df_stock):
    ...
    for idx, stock_row in df_stock.iterrows():
        s_name = str(stock_row.get("商品名稱", "")).upper()
        s_style = str(stock_row.get("商品款式", "")).upper()
        if s_style == "NAN":
            s_style = ""
        stock_name = f"{s_name} {s_style}"
...
def scan_and_merge_purchase_all_columns(file_path):
    ...
    cleaned_columns = [
        f"未命名_{i}" if "__EMPTY_COL_" in str(col) or str(col).strip() == "" else col
...
def start_matching(purchase_path, stock_path):
    ...
    try:
        ...
        for _, p_row in df_purchase.iterrows():
            p_name = p_row.get("貨品名稱", "")
            current_kws = extract_keywords(p_name)

            # 🔍 【品牌型號篩選器】
            clean_brand_model = []
            ignore_details = {
                "WHITE", "BLACK", "RED", "BLUE", "PINK", "YELLOW", "GREEN", "GOLD", "SILVER",
                "NEW", "OLD", "白色", "黑色", "紅色", "藍色", "全新", "二手", "標準", "升級"
            }

            for kw in current_kws:
                # 只保留含有英文或數字的核心字（型號/品牌），並排除顏色狀態雜訊
                if re.search(r"[A-Z0-9]", kw) and kw.upper() not in ignore_details:
                    clean_brand_model.append(kw)

            # 如果篩選完空空如也，才用原有關鍵字頂替（並扣除雜訊字）
            if not clean_brand_model:
                clean_brand_model = [k for k in current_kws if k.upper() not in ignore_details]

            # 用空格精緻結合品牌與型號
            keywords_col.append(" ".join(clean_brand_model))
            ...
            if matched_stock_row is not None:
                ...
                if p_name_str == "" or p_name_str == "nan":
        ...
        # 登記新欄位
        df_purchase["查找關鍵字"] = keywords_col
        ...
        # 重新調整排隊隊伍，把要指定位置的欄位先抽出來
        if "查找關鍵字" in current_cols:
            current_cols.remove("查找關鍵字")
        ...
        if "單位" in current_cols:
            ...
            base_cols = current_cols + cols_to_move + cols_to_end

        # 🔥 決定新版 Excel 的欄位順序：查找關鍵字強制插隊到最左欄
        new_col_order = ["查找關鍵字"] + base_cols
            ...
            for row_idx in range(2, worksheet.max_row + 1):
                ...
                if (
                    ...
                    try:
                        qty_val = (
                            int(float(raw_qty))
                            if pd.notna(raw_qty) and str(raw_qty).strip() != ""
                            else 0
                        )
                        stock_val = (
                            int(float(raw_stock))
                            if pd.notna(raw_stock) and str(raw_stock).strip() != ""
                            else 0
                        )
                    except ValueError:
                        qty_val, stock_val = -1, -2

                    # 3. 當【數量】與【庫存總量】完全相同，整列刷上舒服淡綠色
